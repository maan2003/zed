...
use gpui::{App, BoxShadow, Color as GpuiColor, hsla, px};
...
impl ElevationIndex {
    ...
    pub fn bg(&self, cx: &mut App) -> GpuiColor {
        match self {
    ...
    pub fn on_elevation_bg(&self, cx: &App) -> GpuiColor {
        match self {
    ...
    pub fn darker_bg(&self, cx: &App) -> GpuiColor {
        match self {
            ElevationIndex::Background => cx.theme().colors().surface_background,
