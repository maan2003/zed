...
pub struct SymbolEventSubStepRequest {
    pub symbol_identifier: SymbolIdentifier,
    pub event: SymbolEventSubStep,
}
...
/// Represents different types of Language Learning Entitys (LLMs)
...
impl<T> From<Range> for std::ops::Range<T>
where
    T: From<Position>,
{
...
impl<T> From<std::ops::Range<T>> for Range
where
    Position: From<T>,
{
    fn from(value: std::ops::Range<T>) -> Self {
        Self {
            start_position: value.start.into(),
            end_position: value.end.into(),
        }
    }
}
...
impl From<Position> for lsp::Position {
    fn from(value: Position) -> Self {
        lsp::Position::new(value.line, value.character)
    }
}
...
impl From<text::Point> for Position {
    fn from(value: text::Point) -> Self {
        Self {
            line: value.row,
...
pub struct LLMClientConfig {
    ...
    pub models: HashMap<LLMType, Entity>,
...
pub struct DocumentSymbol {
    ...
    pub selection_range: [lsp::Position; 2],
...
impl From<lsp::DocumentSymbol> for DocumentSymbol {
    fn from(value: lsp::DocumentSymbol) -> Self {
        Self {
            name: value.name,
            detail: value.detail,
            kind: value.kind,
            range: [value.range.start, value.range.end],
            selection_range: [value.selection_range.start, value.selection_range.end],
            children: value
                .children
                .unwrap_or_default()
                .into_iter()
...
pub struct Diagnostic {
    pub message: String,
    pub range: Range,
    pub quick_fix_labels: Option<Vec<String>>,
    pub parameter_hints: Option<Vec<String>>,
    pub fs_file_path: String,
}
