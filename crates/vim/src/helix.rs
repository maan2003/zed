...
use editor::{
    ...
    scroll::Autoscroll,
    Anchor, Bias, DisplayPoint, ToPoint, Editor,
};
...
pub fn register(workspace: &mut Editor, cx: &mut ViewContext<Editor>) {
    workspace.register_action(|_: &mut Editor, _: &Delete, cx: _| delete(cx));
    workspace.register_action(|_: &mut Editor, _: &Change, cx: _| change(cx));
    workspace.register_action(|_: &mut Editor, _: &InsertBefore, cx: _| insert(cx, false));
    workspace.register_action(|_: &mut Editor, _: &InsertAfter, cx: _| insert(cx, true));
    workspace.register_action(|_: &mut Editor, _: &ExtendLineBelow, cx: _| extend_line(cx));
    workspace.register_action(|_: &mut Editor, _: &Yank, cx: _| yank(cx));
    workspace.register_action(|_: &mut Editor, _: &Paste, cx: _| paste(cx));
}
