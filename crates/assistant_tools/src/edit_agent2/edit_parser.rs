...
use std::{mem, ops::Range, path::PathBuf, str::FromStr, sync::Arc};
...
impl XmlEditParser {
    ...
    fn parse_path(&self, tag: &str) -> Option<PathBuf> {
...
impl DiffFencedEditParser {
    ...
    fn parse_path(&self, search_line: &str) -> Option<PathBuf> {
        use regex::Regex;
        use std::sync::LazyLock;
        static PATH_REGEX: LazyLock<Regex> =
            LazyLock::new(|| Regex::new(r#"path=(?:"?)([^">\s]+)"#).unwrap());
